...
class IPWidget:
    ...
    def verificar_atualizacoes2(self):
        self.monitor_id = None
        ...
        gc.collect()
    def verificar_atualizacoes1(self):
        self.monitor_id = None
        ip_novo = get_internal_ip_cache()
        res_novo = (self.root.winfo_screenwidth(), self.root.winfo_screenheight())
        com_novo = get_data_hoje_otimizada()
        ...
        if mudou:
            ...
            # MODO "SÓ IP" - Prioridade: IP > Data > Notícias
            if com_novo:
                # 1º PRIORIDADE: Data comemorativa
                self.label_comemoracao.config(text=f"{com_novo['emoji']} {com_novo['frase']}")
                self.label_comemoracao.pack(pady=2)
                self.ticker.pack_forget()
            else:
                # SEM DATA: Verifica notícias
                self.label_comemoracao.pack_forget()
                noticias = carregar_rss_multiplas_fontes()
                if noticias:
                    # 2º PRIORIDADE: Notícias TI
                    self.ticker.pack(fill='x', pady=(5, 0))
                    if not self.ticker.text:
                        self.ticker.start_news_cycle()
                else:
                    # 3º PRIORIDADE: SÓ IP (esconde tudo)
                    self.label_comemoracao.pack_forget()
                    self.ticker.pack_forget()
    ...
    def verificar_atualizacoes(self):
        ...
        ip_novo = get_internal_ip_cache()
        res_novo = (self.root.winfo_screenwidth(), self.root.winfo_screenheight())
        com_novo = get_data_hoje_otimizada()  # PRIORIDADE 1: Verifica data
        ...
        if mudou:
            ...
            # PRIORIDADE RÍGIDA: IP > DATA > NOTÍCIAS
            if com_novo:
                # ✅ TEM DATA: IP + DATA (notícias escondidas)
                self.label_comemoracao.config(text=f"{com_novo['emoji']} {com_novo['frase']}")
                self.label_comemoracao.pack(pady=2)
                self.ticker.pack_forget()
                print("📅 MODO DATA: IP + Data comemorativa")
            else:
                # ❌ SEM DATA: IP + Notícias (se existirem)
                self.label_comemoracao.pack_forget()
                noticias = carregar_rss_multiplas_fontes()
                if noticias:
                    self.ticker.pack(fill='x', pady=(5, 0))
                    if not self.ticker.text:
                        self.ticker.start_news_cycle()
                    print("📰 MODO NOTÍCIAS: IP + Ticker TI")
                else:
                    self.ticker.pack_forget()
                    print("📍 MODO SÓ IP: Sem data e sem notícias")
    ...
    def redimensionar1(self):
        largura_ip = max(115, len(self.ip_atual) * 20)

        # Em caso de Data Comemorativa
        if self.comemoracao_atual:
            # Com data: 55px de altura
            larg_com = len(f"{self.comemoracao_atual['emoji']} {self.comemoracao_atual['frase']}") * 9
            largura = max(largura_ip, larg_com) + 30
            altura = 45

        # Sem DATA comemorativa porém com noticias ticker visível: 75px de altura
        elif hasattr(self, 'ticker') and self.ticker.winfo_ismapped():

            largura = largura_ip + 30
            altura = 75
        else:
            # SÓ IP: Compacto 35px de altura
    ...
    def redimensionar(self):
        ...
        # IP SEMPRE VISÍVEL - Altura fixa para 2 linhas
        if self.comemoracao_atual:
            # MODO 1: Esta mostrando IP + DATA COMEMORATIVA
            larg_com = len(f"{self.comemoracao_atual['emoji']} {self.comemoracao_atual['frase']}") * 9
            largura = max(largura_ip, larg_com) + 30
            altura = 53  # 2 linhas fixas: Data + IP
            print("IP ")
        elif hasattr(self, 'ticker') and self.ticker.winfo_ismapped():
            # MODO 2: IP + TICKER (2 linhas)
            largura = largura_ip + 30
            altura = 53  # 2 linhas fixas: Ticker + IP
            print("2P")
        else:
            # MODO 3: Esta mostrando IP + Noticias
            largura = largura_ip + 20
            altura = 53  # 1 linha apenas
            print("3P")
